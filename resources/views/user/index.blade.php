<x-dashboard-layout>
    <div class="container py-8 mx-auto">
        <div class="flex justify-between mb-4 items-baseline">
            <h2 class="mb-4 text-2xl font-bold">Daftar User</h2>
                ...
                <tbody class="list">
                    @foreach ($user as $Fadly_user)
                        <tr class="bg-white border-b dark:bg-gray-800 ">
                            ...
                            <td class="px-6 py-4">
                                @if (empty($Fadly_user->NamaLengkap))
                                    <span class="font-bold">. . . . .</span>
                                @else
                                    {{ $Fadly_user->NamaLengkap }}
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if (empty($Fadly_user->Alamat))
                                    <span class="font-bold">. . . . .</span>
                                @else
                                    {{ Str::limit($Fadly_user->Alamat, 10) }}
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                {{ $Fadly_user->roles }}
                            ...
                            <td class="px-6 py-4">
                                @if ($Fadly_user->verified == 0)
                                    <form method="POST" action="{{ route('user.verify', $Fadly_user->UserID) }}">
                                        @csrf
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('user.edit', $Fadly_user) }}"
                                    class="mr-3 font-medium hover:underline">Edit</a>
                                <form action="{{ route('user.destroy', $Fadly_user->UserID) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="font-medium text-black text-bold hover:underline"
                                        onclick="return confirm('Apakah kamu serius ingin menghapus User ini?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
