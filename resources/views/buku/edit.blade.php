<x-dashboard-layout>
    <h1 class="mb-6 text-3xl font-bold">Edit Buku</h1>

    <form action="{{ route('buku.update', $buku->BukuID) }}" method="POST" class="p-6 bg-white rounded-lg shadow-md"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="Judul" class="block mb-2 font-bold text-gray-700">Judul</label>
            <input type="text" name="Judul" id="Judul" class="w-full px-3 py-2 border rounded-lg"
                value="{{ old('Judul', $buku->Judul) }}" required>
        </div>
        <div class="mb-4">
            <label for="Penulis" class="block mb-2 font-bold text-gray-700">Penulis</label>
            <input type="text" name="Penulis" id="Penulis" class="w-full px-3 py-2 border rounded-lg"
                value="{{ old('Penulis', $buku->Penulis) }}" required>
        </div>
        <div class="mb-4">
            <label for="Penerbit" class="block mb-2 font-bold text-gray-700">Penerbit</label>
            <input type="text" name="Penerbit" id="Penerbit" class="w-full px-3 py-2 border rounded-lg"
        ...
        <div class="mb-4">
            ...
            <select name="TahunTerbit" id="TahunTerbit" class="w-full px-3 py-2 border rounded-lg" required>
                <option value="" disabled {{ old('TahunTerbit', $buku->TahunTerbit) ? '' : 'selected' }}>Pilih Tahun Terbit</option>
                @foreach (range(date('Y'), 1900) as $year)
                    <option value="{{ $year }}"
                        {{ old('TahunTerbit', $buku->TahunTerbit) == $year ? 'selected' : '' }}>
                        {{ $year }}
                    </option>
                @endforeach
            </select>
            ...
            @foreach ($kategori as $value)
                <label class="inline-flex items-center mt-2">
                    <input type="checkbox" name="kategori[]" value="{{ $value->KategoriID }}" class="form-checkbox"
                        {{ in_array($value->KategoriID, old('kategori', $buku->kategori()->pluck('KategoriID')->toArray())) ? 'checked' : '' }}>
